# I2C to 7SEGx2 , Beep , GPIO and input interrupt using GreenPAK. (緑豆壱拾陸號)  
...
13 | 8 | INOUT1 | I/O | PU10k | IO8 | GPIO
14 | 7 | VDD2 |  |  | VDD2 |  2.3V～5.5V
15 | 6 | F | O | PU10k | IO9 | segment
16 | 5 | CC2 | O | PU1M | IO10 | Common cathode 2
17 | 4 | CC1 | O | PU10k | IO11 | Common cathode 1
18 | 3 | B | O | PU10k | IO12 | segment
19 | 2 | A | O | PU10k | IO13 | segment
20 | 1 | INOUT2 | I/O | PU10k | IO14 | GPIO

### 内蔵抵抗
各端子の内蔵抵抗を有効にしてありますので、外付け抵抗を省略できます。  
...
0x75 | R | -- | [7:6] | *Reserve*
 ...
 | | | | [4:0] | *Reserve*
0x7A | W | 0x00 | [7] | Interrupt flag mode select<BR>0: OUT4,  1: Interrupt flag mode
 | | | | [6] | Interrupt flag reset<BR>0→1 or 1→0 toggle for reset
 | | | | [5] | Beep mode select<BR>0: OUT3,  1: Beep ON 
 | | | | [4:3] | GPO control<BR>[4]: OUT4  0: output Low, 1: output & Pull-up<BR>[3]: OUT3  0: output Low,  1: output & Pull-up
 | | | | [2:0] | GPIO control<BR>[2]: INOUT2  0: input & Pull-up,  1: output Low<BR>[1]: INOUT1  0: input & Pull-up, 1: output Low<BR>[0]: INOUT0  0: input & Pull-up, 1: output Low
...
0xA5 | W | 0x7C | [7:0] | Beep frequency data. Default: 440Hz<BR>data = 25,000,000/2/frequency[Hz]-1<BR>data[7:0]
0xA6 | W | 0x37 | [7:0] | Beep frequency data<BR>data[15:8]
0xC8 | W | 0x00 | [7:0] | Software reset<BR>0x02:reset
    
## INT
レジスタ 0x7A[7] に1を設定することで、OUT3端子からINT（割り込み）を出力します。  
入力に設定されたGPIO端子の入力値が変化するとHighを出力し、High状態を維持します。  
レジスタ 0x7A[6] の値を書き換えることで、Lowに戻すことができます。  
（前回の値が0なら1を書く、前回の値が1なら0を書く）   
マイコンの割り込み端子やWakeUp端子に接続する使い方を想定しています。  

## BEEP
圧電スピーカーをつなぐことで音を出すことができます。初期値は440Hzです。  
レジスタ 0x7A[5] に1を設定することで、GPO4端子からBEEP信号を出力します。  
レジスタ 0xA5[7:0], 0xA6[7:0] で周波数を変更することができます。  
設定値は、25,000,000/2/周波数(Hz)-1 を計算し、結果を16ビットとして下位8ビットをレジスタ 0xA5[7:0] に、上位8ビットを 0xA6[7:0] に設定します。  
周波数はあまり正確ではありません。  
